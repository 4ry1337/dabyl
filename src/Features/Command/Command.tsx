...
export const Command = () => {
    ...
    useEffect(()=>{
        if(commandError.length === 0 && notificaionError.length === 0 && fileError.length === 0){
            setValid(true)
        }else{
            setValid(false)
        }
        ...
        <>
            <div className={'grow grid grid-cols-2 gap-4'}>
                <div className={'flex flex-col space-y-4'}>
                    <div className={'py-5'}>
                        <Heading className={'uppercase text-center'} lineHeight={'2px'} color={'bg-gray-500'}>Тип сигнала и способ оповещения</Heading>
                    </div>
                    <div className={'flex flex-col grow'}>
                        <div className={'relative grow'}>
                            {isFetching && <Loading/>}
                            {isSuccess && <CommandsList command={command} setCommand={setCommand} commands={commands}/>}
                        </div>
                    </div>
                </div>
                <div className={'flex flex-col space-y-4'}>
            ...
            <div>
                <div className={'py-5'}>
                    <Heading className={'uppercase text-center'} lineHeight={'2px'} color={'bg-gray-500'}>ПРОВЕРКА ОПОЕЩЕНИЯ ПЕРЕД ОТПРАВКОЙ</Heading>
                </div>
                    ...
                    <div className={'col-span-2'}>
                        <div className={'grid grid-rows-3 gap-4'}>
                            <div className={'flex flex-row items-center space-x-4 py-4 px-2.5'}>
                                ...
                                <h1 className={classNames(notificaionError.length === 0 ? 'text-transparent' : 'text-danger', 'grow text-sm font-medium')}>
                                    {notificaionError}
                                </h1>
                            </div>
                            <div className={'bg-secondary py-4 px-2.5 rounded-lg flex flex-row items-center space-x-4'}>
                                <h1 className={'grow h-full'}>{command?.name}</h1>
                                <p className={classNames(commandError.length === 0 ? 'text-transparent' : 'text-danger', 'grow text-end text-sm font-medium')}>
                                    {commandError}
                                </p>
                            ...
                            <div className={'bg-secondary py-4 px-2.5 rounded-lg flex flex-row space-x-4 items-center'}>
                                <h1 className={'grow h-full'}>{file?.name}</h1>
                                <p className={classNames(fileError.length === 0 ? 'text-transparent' : 'text-danger', 'grow text-end text-sm font-medium')}>
                                    {fileError}
                                </p>
                                <div className={'flex flex-row space-x-4 item-center h-full'}>
                                    <HiOutlineTrash className={'h-full w-full'} onClick={()=>setFile(null)}/>
